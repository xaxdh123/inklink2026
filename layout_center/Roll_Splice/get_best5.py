...
✅ 本次额外修改：
- 纸张宽度改为“580左右” => 宽度范围 [570, 590]
- 纸张高度改为 [0, 1080] => 代码实际使用 [1, 1080]
- 刀线长度改为 5mm
...
# ✅ 宽度改成“580左右” => 按 570~590 mm 处理
W_TARGET = 580
W_MIN = 570
W_MAX = 590

# ✅ 高度改成 0~1080（0 实际无意义，代码里用 1 起）
H_MIN, H_MAX = 1, 1080
...
def solve_single_type_template(part_size, need_count):
    """
    ✅ 新规则：
    - 宽度 W_sheet 限制在 [W_MIN, W_MAX]（即 570~590，约580）
    - 高度 H_sheet 在 [H_MIN, H_MAX]（即 1~1080）
    """
    ...
    for ori in (0, 1):
        ...
        for k in range(1, k_upper + 1):
            ...
            if W_sheet < W_MIN:
                continue
            if W_sheet > W_MAX:
                # W_sheet 随 k 增大而增大，这里可直接 break 提速
                break
            ...
            for H_sheet in range(H_MIN, H_MAX + 1):
                ...
                if best is None:
                    ...
                    if cand["total_area"] < best["total_area"] - 1e-9:
                        ...
                        # 2) 宽度更接近 580 优先
                        if cand["w_target_dist"] < best["w_target_dist"] - 1e-9:
                            best = cand
                        elif abs(cand["w_target_dist"] - best["w_target_dist"]) <= 1e-9:
                            # 3) 高度更小优先（更紧凑）
