...
✅ 本次按你的要求额外修改
- 纸张宽度改成“580左右” => 宽度范围 [570, 590]
- 纸张高度改成 [0, 1080] => 代码实际使用 [1, 1080]
- 刀线长度改成 5mm
- 图形与图形之间不画黑线（关闭每个图形外框线）
...
# ✅ 高度改成 0~1080（0 在 PDF 中无意义，这里用 1~1080）
H_MIN, H_MAX = 1, 1080
...
# ✅ 横向有效排版宽度 R = W_sheet - 2*MARGIN，按 570~590 反推
R_MIN = max(1, W_MIN - 2 * MARGIN)
...
# ✅ 轮廓参考页规则
# =========================
def ref_page_index_for_bbox(target_page_index, page_count):
...
# =========================
def build_mark_text_from_type(type_name, block_count):
    base = type_name
    parts = base.split("^")
...
def used_area_from_rows(rows, k_per_row, names_all):
    used = 0
    for nm in names_all:
        r = rows.get(nm, 0)
...
def beam_fill(rows_base, names_all, fill_names, hrow, k_per_row, H_sheet):
...
def best_template_for_N(N, required_names):
    ...
    for ori in enum_orientations(names_all):
        ...
        for R in range(R_MIN, R_MAX + 1, R_STEP):
            ...
            cand = {
                ...
                "w_target_dist": abs(float(W_sheet) - float(W_TARGET)),
            }
            ...
            if best is None or cand["total_area"] < best["total_area"]:
                ...
                # ✅ 同面积优先选宽度更接近580的
                if cand["w_target_dist"] < best.get("w_target_dist", 1e18) - 1e-9:
                    best = cand
                elif abs(cand["w_target_dist"] - best.get("w_target_dist", 1e18)) <= 1e-9:
...
def build_placements(best):
        ...
        for _ in range(int(nrows)):
            ...
            for gi, gsz in enumerate(groups):
                ...
                if gi != len(groups) - 1:
                    x += GAP
            y_inc = rh
            # nonlocal y 在 Python 3 中可直接用，这里保持原风格逻辑
            # 通过外层变量 y 累加
            # noqa-like comment not needed
            # update outer y:
            globals_dummy = None  # 占位，防止误改结构
            _ = globals_dummy
            locals_dummy = None
            _ = locals_dummy
            # 实际更新
            # pylint: disable=undefined-loop-variable
            # 直接使用 nonlocal 已定义的 y
            # 这里不能声明 nonlocal 于子层条件内，所以在函数头定义
            y_holder[0] += y_inc

    # 用 list 包装 y，避免 py2/py3 nonlocal 兼容问题
    y_holder = [y]

    def place_rows_of_type2(nm, nrows):
        groups = best["patterns"][nm]["groups"]
        rh = best["hrow"][nm]
        w = best["z"][nm]
        for _ in range(int(nrows)):
            ...
            for gi, gsz in enumerate(groups):
                for _p in range(gsz):
                    placements.append({"type": nm, "x": x, "y": y_holder[0], "w": w, "h": rh})
                    x += w
                if gi != len(groups) - 1:
                    x += GAP
            y_holder[0] += rh
    ...
    for si, seg in enumerate(segs):
        if len(seg) == 1:
            ...
            run_blocks += 1
            place_rows_of_type2(nm, nrows)

            is_last = (si == nsegs - 1)
            if not is_last:
                labels.append({"type": run_nm, "count": int(run_blocks), "y0": y_holder[0], "y1": y_holder[0] + LABEL_BAND_H})
                y_holder[0] += GAP
                ...
                avail = max(0.0, Huse - y_holder[0])
                band = min(float(LABEL_BAND_H), float(avail))
                if band > 0.5 and run_nm is not None:
                    labels.append({"type": run_nm, "count": int(run_blocks), "y0": y_holder[0], "y1": y_holder[0] + band})
                run_nm = None
                run_blocks = 0
            ...
            for bi, (nm, nrows) in enumerate(seg):
                ...
                if has_next:
                    labels.append({"type": nm, "count": 1, "y0": y_holder[0], "y1": y_holder[0] + LABEL_BAND_H})
                    y_holder[0] += GAP
                else:
                    Huse = H_usable(best["H_sheet"])
                    avail = max(0.0, Huse - y_holder[0])
                    band = min(float(LABEL_BAND_H), float(avail))
                    if band > 0.5:
                        labels.append({"type": nm, "count": 1, "y0": y_holder[0], "y1": y_holder[0] + band})
...
def find_best_with_optional_skips(required_names):
    ...
    def try_find(cur_names):
        ...
        for Ntry in range(2, nmax_dyn + 1):
            ...
            if cand is None:
                continue
            if best_local is None or cand["total_area"] < best_local["total_area"]:
                best_local = cand
            elif best_local is not None and cand["total_area"] == best_local["total_area"]:
                if cand.get("w_target_dist", 1e18) < best_local.get("w_target_dist", 1e18):
                    best_local = cand
        return best_local
