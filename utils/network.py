...
import wmi
from PySide6.QtCore import QObject, QUrl, QStandardPaths, QDir
...
class ApiClient(QObject):
    ...
    def login_mine(self):
        try:
            ...
            if resp.get("code") == 200:
                ...
                self.setting.setValue("auth/mac", mac)
    ...
    def token(self) -> Optional[str]:
        """从缓存中获取 Token"""
        return self.setting.value("auth/token")

    @token.setter
    def token(self, value: str):
        """设置并缓存 Token"""
        self.setting.setValue("auth/token", value)

    def clear_token(self):
        """清除 Token"""
        self.setting.remove("auth/token")
    ...
    def urllib_get(
        ...
        token = self.setting.value("auth/token")
        if token:
    ...
    def d3_post(self, url, data, isT3=False, rpt=0):
        try:
            headers = {
                "Cookie": f'token={self.setting.value("auth/qiyinbz_token" if isT3 else "auth/juheyou_token", "")}',
    ...
    def login(self, isT3=False):
        try:
            ...
            userName = (
                self.setting.value("auth/qiyinbz_username")
                if isT3
                else self.setting.value("auth/juheyou_username")
            )
            userPass = (
                self.setting.value("auth/qiyinbz_user_password")
                if isT3
                else self.setting.value("auth/juheyou_user_password")
            )
            authHost = (
